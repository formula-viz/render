...
from add_driver_objects import add_keyframes

# At 1 meter distance with 50mm lens
TOTAL_WIDTH_COVERED = 0.72  # meters
TOTAL_HEIGHT_COVERED = 0.48  # meters

def setup_status_track_follow(camera_obj, status_track_obj, is_portrait):
    def update_status_track(scene):
        cam_rot = camera_obj.rotation_quaternion if camera_obj.rotation_mode == 'QUATERNION' else camera_obj.rotation_euler

        # Get camera's orientation vectors
        forward_vec = camera_obj.matrix_world.to_quaternion() @ mathutils.Vector((0.0, 0.0, -1.0))
        right_vec = camera_obj.matrix_world.to_quaternion() @ mathutils.Vector((1.0, 0.0, 0.0))
        up_vec = camera_obj.matrix_world.to_quaternion() @ mathutils.Vector((0.0, 1.0, 0.0))
...
def calculate_optimal_scale(new_track_data, camera_obj, is_portrait):
...
def scale(optimal_scale, status_track_obj):
...
def create_indicator_dot():
    bpy.ops.mesh.primitive_uv_sphere_add(radius=10, segments=64, ring_count=64)  # Changed rings to ring_count
...
def get_track_inner_offset(new_track_data, scale):
    ...
    return mathutils.Vector((x_offset, y_offset, 0))

def center(new_track_data, new_driver_df):
    new_inner_points = []
    new_outer_points = []
    ...
    new_inner_points = [(x + offset[0], y + offset[1], z + offset[2]) for x, y, z in new_track_data.inner_points]
    new_outer_points = [(x + offset[0], y + offset[1], z + offset[2]) for x, y, z in new_track_data.outer_points]
    new_inner_curb_points = [(x + offset[0], y + offset[1], z + offset[2]) for x, y, z in new_track_data.inner_curb_points]
    new_outer_curb_points = [(x + offset[0], y + offset[1], z + offset[2]) for x, y, z in new_track_data.outer_curb_points]
    ...
    new_track_data = TrackData(new_inner_points, new_outer_points, new_inner_curb_points, new_outer_curb_points)

    return new_track_data, new_driver_df

# new_driver_df here is the df for the driver that becomes the indicator dot
def main(track_data, camera_obj, start_finish_line_idx, driver_df):
    is_portrait = True

    new_track_data, new_driver_df = center(track_data, driver_df)

    optimal_scale = calculate_optimal_scale(new_track_data, camera_obj, is_portrait)

    track_collection = bpy.data.collections.new(name="StatusTrackCollection")
    ...
    status_track_obj = create_planes(new_track_data.inner_points, new_track_data.outer_points, "Status", track_mat)
    status_start_finish_line = add_start_finish_line(new_track_data.inner_points, new_track_data.outer_points, start_finish_line_idx, "StatusStartFinishLine", line_width=20)

    scale(optimal_scale, status_track_obj)
    ...
    setup_status_track_follow(camera_obj, status_track_obj, is_portrait)
