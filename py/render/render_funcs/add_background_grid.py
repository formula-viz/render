...
def create_grid_curves(index: int = 0, size: int = 1000, spacing: float = 1.0, offset: tuple = (0, 0, 0)):
    ...
    for i in range(-size//2, size//2 + 1):
        ...
        pos = i * spacing
        spline.points[0].co = (-size*spacing/2 + offset[0],
                               pos + offset[1], -10 + offset[2], 1)
        spline.points[1].co = (size*spacing/2 + offset[0],
                               pos + offset[1], -10 + offset[2], 1)

        # Y direction
        spline = curve_data.splines.new('POLY')
        spline.points.add(1)
        spline.points[0].co = (pos + offset[0], -size *
                               spacing/2 + offset[1], -10 + offset[2], 1)
        spline.points[1].co = (pos + offset[0], size *
                               spacing/2 + offset[1], -10 + offset[2], 1)
...
def main():
    ...
    grid_obj1 = create_grid_curves(
        index=1, size=1000, spacing=3.0, offset=(0, 0, 0))
    ...
    grid_obj1.data.materials.append(grid_material1)

    grid_material2 = create_grid_material((0.4, 0.4, 0.4, 1))  # gray
    grid_obj2.data.materials.append(grid_material2)
