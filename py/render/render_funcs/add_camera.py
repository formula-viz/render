...
def add_keyframes(camera_obj, cam_df, driver_df, start_buffer_frames, end_buffer_frames):
    ...
    for i in range(len(cam_df)):
        ...
        if start_buffer_frames <= frame <= len(cam_df) - end_buffer_frames:
            look_at_point = mathutils.Vector((driver_df["X"][i], driver_df["Y"][i], driver_df["Z"][i]))
